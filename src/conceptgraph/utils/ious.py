...
def compute_axis_aligned_inter_vol(
    bbox1: torch.Tensor, bbox2: torch.Tensor
) -> torch.Tensor:
    """
    Axis-aligned intersection volume between two sets of 3D boxes given by corners.

    Used as a fallback for pytorch3d's oriented-box overlap when the compiled
    pytorch3d ops are unavailable.

    bbox1: (M, 8, 3)
    bbox2: (N, 8, 3)

    returns: (M, N) intersection volume
    """
    bbox1_min = bbox1.min(dim=1).values.unsqueeze(1)  # (M, 1, 3)
    bbox1_max = bbox1.max(dim=1).values.unsqueeze(1)  # (M, 1, 3)
    bbox2_min = bbox2.min(dim=1).values.unsqueeze(0)  # (1, N, 3)
    bbox2_max = bbox2.max(dim=1).values.unsqueeze(0)  # (1, N, 3)

    inter_min = torch.max(bbox1_min, bbox2_min)  # (M, N, 3)
    inter_max = torch.min(bbox1_max, bbox2_max)  # (M, N, 3)
...
def compute_3d_iou_accurate_batch(bbox1, bbox2):
    ...
    try:
        ...
        # pytorch3d compiled ops unavailable (e.g. CUDA/torch ABI mismatch).
        # Fall back to the axis-aligned IoU, which is sufficient here since the
        # accurate IoU is only used as a coarse pre-filter / spatial gate.
...
def compute_3d_giou_accurate_batch(
    ...
    try:
        ...
        # Fall back to axis-aligned intersection when pytorch3d ops are unavailable.
        inter_vol = compute_axis_aligned_inter_vol(bbox1, bbox2)
        iou = inter_vol / (
            bbox1_vol.unsqueeze(1) + bbox2_vol.unsqueeze(0) - inter_vol + 1e-10
        )
